...
isbn_validador = RegexValidator(
    regex=r"^(?:\d{9}X|\d{10}|\d{13})$", message="Código ISBN inválido"
)


class Livro(models.Model):
    CLAS_CHOICES = [("livre", "livre para todos"), ("adulto", "adulto")]
    ...
    isbn = models.CharField(
        max_length=13, validators=[isbn_validador], blank=True, null=True
    )
    ...
    publicacao = models.DateField(
        verbose_name="data de publicação", blank=True, null=True
    )

    tipo_capa = models.CharField(
        max_length=45, choices=CAPA_CHOICES, blank=True, null=True
    )
    faixa_etaria = models.CharField(
        max_length=45, choices=CLAS_CHOICES, blank=True, null=True
    )

    editora = models.ForeignKey(
        Editora, on_delete=models.PROTECT, related_name="livro", null=True, blank=True
    )
